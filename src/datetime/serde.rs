...
impl<Tz: TimeZone> ser::Serialize for DateTime<Tz> {
    ...
    S: ser::Serializer,
  {
    ...
    serializer.collect_str(&FormatIso8601 {
      inner: self
    })
  }
}
...
pub mod ts_nanoseconds {
    ...
    S: ser::Serializer,
  {
    serializer.serialize_i64(
      dt.timestamp_nanos_opt()
        .ok_or(ser::Error::custom("value out of range for a timestamp with nanosecond precision"))?,
    )
  }
  ...
  impl de::Visitor<'_> for NanoSecondsTimestampVisitor {
    ...
    where
      E: de::Error,
    {
      DateTime::from_timestamp((value / 1_000_000_000) as i64, (value % 1_000_000_000) as u32).ok_or_else(|| invalid_ts(value))
    }
  }
}
...
pub mod ts_nanoseconds_option {
  ...
  {
    match *opt {
      Some(ref dt) => serializer.serialize_some(
        &dt
          .timestamp_nanos_opt()
          .ok_or(ser::Error::custom("value out of range for a timestamp with nanosecond precision"))?,
      ),
...
pub mod ts_microseconds {
  ...
  impl de::Visitor<'_> for MicroSecondsTimestampVisitor {
    ...
    where
      E: de::Error,
    {
      DateTime::from_timestamp((value / 1_000_000) as i64, ((value % 1_000_000) * 1_000) as u32).ok_or_else(|| invalid_ts(value))
    }
  }
}
...
pub mod ts_milliseconds {
  ...
  impl de::Visitor<'_> for MilliSecondsTimestampVisitor {
    ...
    where
      E: de::Error,
    {
      DateTime::from_timestamp((value / 1000) as i64, ((value % 1000) * 1_000_000) as u32).ok_or_else(|| invalid_ts(value))
    }
  }
}
...
mod tests {
  ...
  fn test_serde_deserialize() {
    ...
    let dt: Option<DateTime<FixedOffset>> = serde_json::from_str(r#""2014-07-24T12:34:06Z""#).ok();
    assert_eq!(
      norm(&dt),
      norm(&Some(
    ...
    let dt: Option<DateTime<FixedOffset>> = serde_json::from_str(r#""2014-07-24T13:57:06+01:23""#).ok();
    assert_eq!(
      norm(&dt),
      norm(&Some(
    ...
    #[cfg(feature = "clock")]
    {
      let dt: DateTime<Local> = serde_json::from_str(r#""2014-07-24T12:34:06Z""#).expect("local should parse");
      assert_eq!(dt, Utc.with_ymd_and_hms(2014, 7, 24, 12, 34, 6).unwrap());

      let dt: DateTime<Local> = serde_json::from_str(r#""2014-07-24T13:57:06+01:23""#).expect("local should parse with offset");
      assert_eq!(dt, Utc.with_ymd_and_hms(2014, 7, 24, 12, 34, 6).unwrap());
    }
  ...
  fn test_serde_no_offset_debug() {
    ...
    impl TimeZone for TestTimeZone {
      ...
      fn offset_from_local_datetime(&self, _local: &NaiveDateTime) -> MappedLocalTime<TestTimeZone> {
        MappedLocalTime::Single(TestTimeZone)
      }
