...
impl<Tz: TimeZone> DateTime<Tz> {
  ...
  /// let other: DateTime<FixedOffset> = FixedOffset::east_opt(23)
  ///   .unwrap()
  ...
  pub fn to_rfc2822(&self) -> String {
    ...
    write_rfc2822(&mut result, self.overflowing_naive_local(), self.offset.fix()).expect("date cannot be represented by RFC 2822");
    result
  }
  ...
  pub fn to_rfc3339(&self) -> String {
    ...
    write_rfc3339(&mut result, naive, offset, SecondsFormat::AutoSi, false).expect("writing rfc3339 datetime to string should never fail");
    result
  }
...
impl DateTime<Utc> {
  ...
  ///   v.into_iter().map(DateTime::from_timestamp_secs).collect();
  ///
  /// assert_eq!(
  ///   vec![
  ///     None,
...
impl DateTime<FixedOffset> {
  ...
  ///   [Appendix A.5]
  /// - Single letter 'military' time zone names are parsed as a `-0000` offset. They were defined
  ///   with the wrong sign in RFC 822 and corrected in RFC 2822. But because the meaning is now
  ///   ambiguous, the standard says they should be considered as `-0000` unless there is
  ///   out-of-band information confirming their meaning. The exception is `Z`, which remains
  ///   identical to `+0000`.
  ///
  ...
  Tz::Offset: fmt::Display,
{
  ...
  {
    let local = self.overflowing_naive_local();
    DelayedFormat::new_with_offset_and_locale(Some(local.date()), Some(local.time()), &self.offset, items, locale)
  }
