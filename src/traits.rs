...
/// - Don't combine two `with_*` methods to change two components of the date. For example to change
///   both the year and month components of a date. This could fail because an intermediate value
///   does not exist, while the final date would be valid.
///
...
pub trait Datelike: Sized {
  ...
  /// - In case of [`DateTime<Tz>`] if the resulting date and time fall within a timezone transition
  ///   such as from DST to standard time.
  ///
  /// [`NaiveDate`]: crate::NaiveDate
...
mod tests {
  ...
  fn test_num_days_from_ce_against_alternative_impl() {
    ...
    for year in NaiveDate::MIN.year()..=NaiveDate::MAX.year() {
      ...
      assert_eq!(jan1_year.num_days_from_ce(), num_days_from_ce(&jan1_year), "on {jan1_year:?}");
