...
use super::win_bindings::GetTimeZoneInformationForYear;
use super::win_bindings::SYSTEMTIME;
use super::win_bindings::TIME_ZONE_INFORMATION;
use crate::Datelike;
...
use crate::NaiveTime;
use crate::Weekday;
use crate::offset::local::Transition;
use crate::offset::local::lookup_with_dst_transitions;
...
pub(super) fn offset_from_local_datetime(local: &NaiveDateTime) -> MappedLocalTime<FixedOffset> {
  ...
  match (tz_info.std_transition, tz_info.dst_transition) {
    (Some(std_transition), Some(dst_transition)) => {
      let std_transition = Transition::new(std_transition, tz_info.dst_offset, tz_info.std_offset);
      let dst_transition = Transition::new(dst_transition, tz_info.std_offset, tz_info.dst_offset);
    ...
    (Some(std_transition), None) => {
      let transitions = [Transition::new(std_transition, tz_info.dst_offset, tz_info.std_offset)];
      lookup_with_dst_transitions(&transitions, *local)
    }
    (None, Some(dst_transition)) => {
      let transitions = [Transition::new(dst_transition, tz_info.std_offset, tz_info.dst_offset)];
      lookup_with_dst_transitions(&transitions, *local)
    }
...
fn naive_date_time_from_system_time(st: SYSTEMTIME, year: i32) -> Result<Option<NaiveDateTime>, ()> {
  if st.wYear == 0 && st.wMonth == 0 {
    return Ok(None);
  }
  let time = NaiveTime::from_hms_milli_opt(st.wHour as u32, st.wMinute as u32, st.wSecond as u32, st.wMilliseconds as u32).ok_or(())?;

  if st.wYear != 0 {
    // We have a concrete date.
    let date = NaiveDate::from_ymd_opt(st.wYear as i32, st.wMonth as u32, st.wDay as u32).ok_or(())?;
...
mod tests {
  ...
  use crate::TimeZone;
  use crate::Timelike;
  use crate::offset::local::win_bindings::FILETIME;
  use crate::offset::local::win_bindings::SYSTEMTIME;
  use crate::offset::local::win_bindings::SystemTimeToFileTime;
  use crate::offset::local::win_bindings::TzSpecificLocalTimeToSystemTime;
