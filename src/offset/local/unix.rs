...
fn offset(d: &NaiveDateTime, local: bool) -> MappedLocalTime<FixedOffset> {
  TZ_INFO.with(|maybe_cache| maybe_cache.borrow_mut().get_or_insert_with(Cache::default).offset(*d, local))
}
...
impl Cache {
  fn offset(&mut self, d: NaiveDateTime, local: bool) -> MappedLocalTime<FixedOffset> {
    ...
    // we pass through the year as the year of a local point in time must either be valid in that
    // locale, or the entire time was skipped in which case we will return MappedLocalTime::None
    // anyway.
    self
      .zone
