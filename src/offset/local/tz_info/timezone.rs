...
impl TimeZone {
  ...
  fn from_posix_tz(tz_string: &str) -> Result<Self, Error> {
    ...
    Self::new(
      ...
      match rule {
        ...
        TransitionRule::Alternate(AlternateTime {
          std,
          dst,
          ..
        }) => vec![std, dst],
  ...
  pub(crate) fn find_local_time_type_from_local(&self, local_time: NaiveDateTime) -> Result<crate::MappedLocalTime<LocalTimeType>, Error> {
        ...
        if unix_leap_time >= last_transition.unix_leap_time {
          match self.extra_rule {
            ...
            None => {
              ...
              return Ok(&self.local_time_types[last_transition.local_time_type_index]);
            }
          }
        } else {
  ...
  pub(crate) fn find_local_time_type_from_local(&self, local_time: NaiveDateTime) -> Result<crate::MappedLocalTime<LocalTimeType>, Error> {
    ...
    let offset_after_last = if !self.transitions.is_empty() {
      ...
      for transition in self.transitions {
        ...
        let transition_end = transition.unix_leap_time.saturating_add(i64::from(after_ltt.ut_offset));
        let transition_start = transition.unix_leap_time.saturating_add(i64::from(prev.ut_offset));
  ...
  fn validate(&self) -> Result<(), Error> {
    ...
    while i_transition < self.transitions.len() {
      ...
      if i_transition + 1 < self.transitions.len()
        && self.transitions[i_transition].unix_leap_time >= self.transitions[i_transition + 1].unix_leap_time
      {
    ...
    if !check {
      return Err(Error::TimeZone("extra transition rule is inconsistent with the last transition"));
    }
...
mod tests {
  ...
  fn test_find_local_time_type_from_local_overflow() -> Result<(), Error> {
    ...
    let local = crate::NaiveDate::from_ymd_opt(2020, 1, 1)
      .unwrap()
      .and_hms_opt(0, 0, 0)
      .unwrap();
