...
use super::CUMUL_DAY_IN_MONTHS_NORMAL_YEAR;
use super::DAY_IN_MONTHS_NORMAL_YEAR;
use super::DAYS_PER_WEEK;
use super::Error;
use super::SECONDS_PER_DAY;
use super::parser::Cursor;
use super::timezone::LocalTimeType;
use super::timezone::SECONDS_PER_WEEK;
use crate::Datelike;
...
impl TransitionRule {
  ...
  /// Find the local time type associated to the transition rule at the specified Unix time in
  /// seconds
  pub(super) fn find_local_time_type(&self, unix_time: i64) -> Result<&LocalTimeType, Error> {
    match self {
      ...
      TransitionRule::Alternate(alternate_time) => alternate_time.find_local_time_type(unix_time),
    }
  }

  /// Find the local time type associated to the transition rule at the specified Unix time in
  /// seconds
  pub(super) fn find_local_time_type_from_local(&self, local_time: NaiveDateTime) -> Result<crate::MappedLocalTime<LocalTimeType>, Error> {
    match self {
      TransitionRule::Fixed(local_time_type) => Ok(crate::MappedLocalTime::Single(*local_time_type)),
      TransitionRule::Alternate(alternate_time) => alternate_time.find_local_time_type_from_local(local_time),
    }
  }
}
...
impl AlternateTime {
  ...
  /// Find the local time type associated to the alternate transition rule at the specified Unix
  /// time in seconds
  fn find_local_time_type(&self, unix_time: i64) -> Result<&LocalTimeType, Error> {
    ...
    // Check DST start/end Unix times for previous/current/next years to support for transition day
    // times outside of [0h, 24h] range
    let is_dst = match Ord::cmp(&current_year_dst_start_unix_time, &current_year_dst_end_unix_time) {
  ...
  fn find_local_time_type_from_local(&self, local_time: NaiveDateTime) -> Result<crate::MappedLocalTime<LocalTimeType>, Error> {
    ...
    let dst_start_transition_start = self.dst_start.unix_time(current_year, 0) + i64::from(self.dst_start_time);
    let dst_start_transition_end =
      self.dst_start.unix_time(current_year, 0) + i64::from(self.dst_start_time) + i64::from(self.dst.ut_offset)
        - i64::from(self.std.ut_offset);

    let dst_end_transition_start = self.dst_end.unix_time(current_year, 0) + i64::from(self.dst_end_time);
    let dst_end_transition_end = self.dst_end.unix_time(current_year, 0) + i64::from(self.dst_end_time) + i64::from(self.std.ut_offset)
    ...
    match self.std.ut_offset.cmp(&self.dst.ut_offset) {
      ...
      Ordering::Less => {
        if self.dst_start.transition_date(current_year).0 < self.dst_end.transition_date(current_year).0 {
          // northern hemisphere
          ...
          } else if local_time > dst_start_transition_start && local_time < dst_start_transition_end {
...
#[derive(Debug, Copy, Clone, Eq, PartialEq)]
enum RuleDay {
  /// Julian day in `[1, 365]`, without taking occasional Feb 29 into account, which is not
  /// referenceable
...
impl RuleDay {
  ...
  fn parse(cursor: &mut Cursor, use_string_extensions: bool) -> Result<(Self, i32), Error> {
    ...
    Ok((date, match (cursor.read_optional_tag(b"/")?, use_string_extensions) {
      (false, _) => 2 * 3600,
  ...
  /// occasional Feb 29 into account, which is not referenceable
  ...
  /// Construct a transition rule day represented by a zero-based Julian day in `[0, 365]`, taking
  /// occasional Feb 29 into account
  ...
  fn transition_date(&self, year: i32) -> (usize, i64) {
    match *self {
      ...
      RuleDay::MonthWeekday {
        ...
        let first_week_day_occurrence_in_month = 1 + (week_day as i64 - week_day_of_first_month_day).rem_euclid(DAYS_PER_WEEK);

        let mut month_day = first_week_day_occurrence_in_month + (week as i64 - 1) * DAYS_PER_WEEK;
...
const DAY_IN_MONTHS_LEAP_YEAR_FROM_MARCH: [i64; 12] = [31, 30, 31, 30, 31, 31, 30, 31, 30, 31, 31, 29];
...
mod tests {
  ...
  fn test_transition_rule() -> Result<(), Error> {
    ...
    assert_eq!(transition_rule_negative_time_2.find_local_time_type(954032399)?.offset(), -10800);
    assert_eq!(transition_rule_negative_time_2.find_local_time_type(954032400)?.offset(), -7200);
    assert_eq!(transition_rule_negative_time_2.find_local_time_type(972781199)?.offset(), -7200);
    assert_eq!(transition_rule_negative_time_2.find_local_time_type(972781200)?.offset(), -10800);
