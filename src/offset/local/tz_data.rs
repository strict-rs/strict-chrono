...
fn find_tz_data<const ENTRY_LEN: usize>(mut reader: impl Read + Seek, tz_name: &[u8]) -> Result<Option<Vec<u8>>> {
...
impl TzDataIndexes {
      ...
      indexes: buf
        ...
        .filter_map(|chunk| {
          from_bytes_until_nul(&chunk[..TZ_NAME_LEN]).map(|name| {
            ...
            let offset = u32::from_be_bytes(chunk[TZ_NAME_LEN..TZ_NAME_LEN + 4].try_into().unwrap());
            let length = u32::from_be_bytes(chunk[TZ_NAME_LEN + 4..TZ_NAME_LEN + 8].try_into().unwrap());
            TzDataIndex {
              name,
              offset,
              length,
  ...
  fn find_tzdata(&self, mut reader: impl Read + Seek, header: &TzDataHeader, index: &TzDataIndex) -> Result<Vec<u8>> {
...
fn from_bytes_until_nul(bytes: &[u8]) -> Option<&CStr> {
  ...
  // 2. We know there is a nul byte at nul_pos, so this slice (ending at the nul byte) is a
...
mod tests {
  ...
  fn test_tzdata_index_partial_entry() {
    ...
    let indexes = TzDataIndexes::new::<OHOS_ENTRY_LEN>(&mut Cursor::new(&data[24..]), &header).unwrap();
