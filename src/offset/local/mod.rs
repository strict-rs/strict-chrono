...
mod inner {
  ...
  pub(super) fn offset_from_utc_datetime(_utc_time: &NaiveDateTime) -> MappedLocalTime<FixedOffset> {
    MappedLocalTime::Single(FixedOffset::east_opt(0).unwrap())
  }

  pub(super) fn offset_from_local_datetime(_local_time: &NaiveDateTime) -> MappedLocalTime<FixedOffset> {
    MappedLocalTime::Single(FixedOffset::east_opt(0).unwrap())
  }
}
...
#[cfg(windows)]
impl Transition {
  fn new(transition_local: NaiveDateTime, offset_before: FixedOffset, offset_after: FixedOffset) -> Transition {
...
#[cfg(windows)]
fn lookup_with_dst_transitions(transitions: &[Transition], dt: NaiveDateTime) -> MappedLocalTime<FixedOffset> {
  for t in transitions.iter() {
    ...
    // space around the `NaiveDateTime` range (although it is very theoretical to have a
    // transition at midnight around `NaiveDate::(MIN|MAX)`.
    let (offset_min, offset_max) = match t.offset_after.local_minus_utc() > t.offset_before.local_minus_utc() {
...
mod tests {
  ...
  fn test_leap_second() {
    ...
    if let Some(dt) = today.and_hms_milli_opt(15, 2, 59, 1000) {
      ...
      assert!(timestr == "15:02:60" || timestr == "15:03:00", "unexpected timestr {timestr:?}");
    }
  ...
  fn test_lookup_with_dst_transitions() {
    ...
    fn compare_lookup(transitions: &[Transition], y: i32, m: u32, d: u32, h: u32, n: u32, s: u32, result: MappedLocalTime<FixedOffset>) {
