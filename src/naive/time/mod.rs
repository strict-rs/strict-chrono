...
/// - `03:01:00 - 03:00:60.5 = 0.5s`. Note that the difference is larger than the previous, even
///   though the leap second clearly follows the previous whole second.
/// - `04:00:60.9 - 03:00:60.1 = (04:00:60.9 - 04:00:00) + (04:00:00 - 03:01:00) + (03:01:00 -
///   03:00:60.1) = 60.9s + 3540s + 0.9s = 3601.8s`.
///
/// In general,
///
...
///
/// - Associativity does not generally hold, because `(Time + TimeDelta1) - TimeDelta2` no longer
///   equals to `Time + (TimeDelta1 - TimeDelta2)` for two positive durations.
///
///     - As a special case, `(Time + TimeDelta) - TimeDelta` also does not equal to `Time`.
///
///     - If you can assume that all durations have the same sign, however, then the associativity
///       holds: `(Time + TimeDelta1) + TimeDelta2` equals to `Time + (TimeDelta1 + TimeDelta2)` for
///       two positive durations.
///
...
/// let leap_sec_2015 = NaiveDate::from_ymd_opt(2015, 6, 30)
///   .unwrap()
///   .and_hms_milli_opt(23, 59, 59, 1_000)
///   .unwrap();
...
impl NaiveTime {
  ...
  pub const fn from_hms_nano_opt(hour: u32, min: u32, sec: u32, nano: u32) -> Option<NaiveTime> {
    if (hour >= 24 || min >= 60 || sec >= 60) || (nano >= 1_000_000_000 && sec != 59) || nano >= 2_000_000_000 {
      return None;
    }
  ...
  ///
  /// assert_eq!(
  ///   since(from_hmsm(3, 5, 7, 900), from_hmsm(3, 5, 7, 900)),
  ///   TimeDelta::zero()
  /// );
  /// assert_eq!(
...
impl Timelike for NaiveTime {
  ...
  /// assert_eq!(
  ///   NaiveTime::from_hms_nano_opt(23, 56, 4, 12_345_678).unwrap().hour(),
  ///   23
  /// );
  /// ```
  #[inline]
  ...
  /// assert_eq!(
  ///   NaiveTime::from_hms_nano_opt(23, 56, 4, 12_345_678).unwrap().minute(),
  ///   56
  /// );
  /// ```
  #[inline]
  ...
  /// assert_eq!(
  ///   NaiveTime::from_hms_nano_opt(23, 56, 4, 12_345_678).unwrap().second(),
  ///   4
  /// );
  /// ```
  ///
  ...
  /// assert_eq!(
  ///   leap.format("%H:%M:%S%.9f").to_string(),
  ///   "23:59:60.000000000"
  /// );
  /// # }
  /// ```
  ...
  /// assert_eq!(
  ///   dt.with_hour(7),
  ///   Some(NaiveTime::from_hms_nano_opt(7, 56, 4, 12_345_678).unwrap())
  /// );
  ...
  ///
  /// assert_eq!(
  ///   NaiveTime::from_hms_opt(1, 2, 3).unwrap().num_seconds_from_midnight(),
  ///   3723
  /// );
  /// assert_eq!(
  ///   NaiveTime::from_hms_nano_opt(23, 56, 4, 12_345_678)
  ///     .unwrap()
...
///
/// assert_eq!(
///   from_hmsm(3, 5, 7, 0) + TimeDelta::zero(),
///   from_hmsm(3, 5, 7, 0)
/// );
/// assert_eq!(
///   from_hmsm(3, 5, 7, 0) + TimeDelta::try_seconds(1).unwrap(),
///   from_hmsm(3, 5, 8, 0)
/// );
/// assert_eq!(
///   from_hmsm(3, 5, 7, 0) + TimeDelta::try_seconds(-1).unwrap(),
...
///
/// assert_eq!(
///   from_hmsm(3, 5, 7, 900) - from_hmsm(3, 5, 7, 900),
///   TimeDelta::zero()
/// );
/// assert_eq!(
...
///
/// assert_eq!(
///   format!("{:?}", NaiveTime::from_hms_opt(23, 56, 4).unwrap()),
///   "23:56:04"
/// );
/// assert_eq!(
///   format!(
///     "{:?}",
///     NaiveTime::from_hms_milli_opt(23, 56, 4, 12).unwrap()
///   ),
///   "23:56:04.012"
/// );
/// assert_eq!(
///   format!(
///     "{:?}",
///     NaiveTime::from_hms_micro_opt(23, 56, 4, 1234).unwrap()
///   ),
///   "23:56:04.001234"
/// );
/// assert_eq!(
///   format!(
///     "{:?}",
///     NaiveTime::from_hms_nano_opt(23, 56, 4, 123456).unwrap()
///   ),
///   "23:56:04.000123456"
/// );
/// ```
...
///   format!(
///     "{:?}",
///     NaiveTime::from_hms_milli_opt(6, 59, 59, 1_500).unwrap()
///   ),
///   "06:59:60.500"
/// );
/// ```
