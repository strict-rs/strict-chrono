...
pub mod ts_nanoseconds {
  ...
  impl de::Visitor<'_> for NanoSecondsTimestampVisitor {
    ...
    where
      E: de::Error,
    {
      DateTime::from_timestamp(value.div_euclid(1_000_000_000), (value.rem_euclid(1_000_000_000)) as u32)
        .map(|dt| dt.naive_utc())
...
pub mod ts_microseconds {
  ...
  impl de::Visitor<'_> for MicroSecondsTimestampVisitor {
    ...
    where
      E: de::Error,
    {
      DateTime::from_timestamp((value / 1_000_000) as i64, ((value % 1_000_000) * 1_000) as u32)
        .map(|dt| dt.naive_utc())
        .ok_or_else(|| invalid_ts(value))
    }
  }
...
mod tests {
  ...
  fn test_serde_serialize() {
    ...
    assert_eq!(
      serde_json::to_string(&NaiveDate::from_ymd_opt(2014, 7, 24).unwrap().and_hms_opt(12, 34, 6).unwrap()).ok(),
