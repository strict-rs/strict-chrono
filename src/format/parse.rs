...
use super::Fixed;
use super::INVALID;
use super::InternalFixed;
use super::InternalInternal;
use super::Item;
use super::Numeric;
...
fn parse_rfc2822<'a>(parsed: &mut Parsed, mut s: &'a str) -> ParseResult<(&'a str, ())> {
  ...
  //   year should be interpreted by adding 1900. note that four-or-more-digit years less than 1000
...
pub(crate) fn parse_rfc3339(mut s: &str) -> ParseResult<DateTime<FixedOffset>> {
  ...
  // - quoted characters can be in any mixture of lower and upper cases.
  //
  // - it may accept any number of fractional digits for seconds. for Chrono, this means that we
  //   should skip digits past first 9 digits.
  //
  // - unlike RFC 2822, the valid offset ranges from -23:59 to +23:59. note that this restriction is
  //   unique to RFC 3339 and not ISO 8601. since this is not a typical Chrono behavior, we check it
  //   earlier.
  //
  ...
  let year = digit(fixed, 0)? as u16 * 1000 + digit(fixed, 1)? as u16 * 100 + digit(fixed, 2)? as u16 * 10 + digit(fixed, 3)? as u16;
  ...
  let date = NaiveDate::from_ymd_opt(year as i32, month as u32, day as u32).ok_or(OUT_OF_RANGE)?;
  ...
  let time = NaiveTime::from_hms_nano_opt(hour as u32, min as u32, sec as u32, nano).ok_or(OUT_OF_RANGE)?;
  ...
  Ok(match date.and_time(time).and_local_timezone(tz) {
    ...
    MappedLocalTime::Ambiguous(..) | MappedLocalTime::None => unreachable!(),
  })
}
...
///
/// - Greedy. It will consume the longest possible prefix. For example, `April` is always consumed
///   entirely when the long month name is requested; it equally accepts `Apr`, but prefers the
///   longer prefix in this case.
///
/// - Padding-agnostic (for numeric items). The [`Pad`](./enum.Pad.html) field is completely
///   ignored, so one can prepend any number of whitespace then any number of zeroes before numbers.
///
...
///
/// - Padding-agnostic (for numeric items). The [`Pad`](./enum.Pad.html) field is completely
///   ignored, so one can prepend any number of zeroes before numbers.
///
/// - (Still) obeying the intrinsic parsing width. This allows, for example, parsing `HHMMSS`.
pub fn parse_and_remainder<'a, 'b, I, B>(parsed: &mut Parsed, s: &'b str, items: I) -> ParseResult<&'b str>
where
  I: Iterator<Item = B>,
  B: Borrow<Item<'a>>,
{
...
fn parse_internal<'a, 'b, I, B>(parsed: &mut Parsed, mut s: &'b str, items: I) -> Result<&'b str, ParseError>
where
  I: Iterator<Item = B>,
  B: Borrow<Item<'a>>,
{
    ...
    match *item.borrow() {
      ...
      Item::Fixed(ref spec) => {
        ...
        match spec {
          ...
          &TimezoneOffsetColon | &TimezoneOffsetDoubleColon | &TimezoneOffsetTripleColon | &TimezoneOffset => {
            let offset = try_consume!(scan::timezone_offset(s.trim_start(), scan::colon_or_space, false, false, true,));
            parsed.set_offset(i64::from(offset))?;
          }

          &TimezoneOffsetColonZ | &TimezoneOffsetZ => {
            let offset = try_consume!(scan::timezone_offset(s.trim_start(), scan::colon_or_space, true, false, true,));
            parsed.set_offset(i64::from(offset))?;
          }
          &Internal(InternalFixed {
            val: InternalInternal::TimezoneOffsetPermissive,
          }) => {
            let offset = try_consume!(scan::timezone_offset(s.trim_start(), scan::colon_or_space, true, true, true,));
            parsed.set_offset(i64::from(offset))?;
          }
...
mod tests {
  ...
  fn test_parse_fixed_nanosecond() {
    ...
    use crate::format::Fixed::Nanosecond3;
    use crate::format::Fixed::Nanosecond6;
    use crate::format::Fixed::Nanosecond9;
  ...
  fn test_rfc2822() {
    ...
    let testdates = [
      ...
      ("Tue, 20 Jan 2015 17:35:20 ZULU", Err(INVALID)),       // bad offset: zulu not allowed
      ("Tue, 20 Jan 2015 17:35:20 −0800", Err(INVALID)),      /* bad offset: timezone offset using MINUS SIGN (U+2212), not specified
                                                               * for RFC 2822 */
