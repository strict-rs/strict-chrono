...
use crate::offset::MappedLocalTime;
...
///
/// - `set_*` methods to set fields you have available. They do a basic range check, and if the same
///   field is set more than once it is checked for consistency.
///
/// - `to_*` methods try to make a concrete date and time value out of set fields. They fully check
///   that all fields are consistent and whether the date/datetime exists.
///
...
impl Parsed {
  ...
  ///
  /// This method returns:
  /// - `OUT_OF_RANGE` if any of the time fields of `Parsed` are set to a value beyond their
  ///   acceptable range.
  /// - `NOT_ENOUGH` if an hour field is missing, if AM/PM is missing in a 12-hour clock, if minutes
  ///   are missing, or if seconds are missing while the nanosecond field is present.
  ...
  pub fn to_naive_datetime_with_offset(&self, offset: i32) -> ParseResult<NaiveDateTime> {
    ...
    if let (Ok(date), Ok(time)) = (date, time) {
      ...
      if let Some(given_timestamp) = self.timestamp {
        ...
        if given_timestamp != timestamp && !(datetime.nanosecond() >= 1_000_000_000 && given_timestamp == timestamp + 1) {
          return Err(IMPOSSIBLE);
        }
      }
      ...
      use super::ParseErrorKind::Impossible;
  ...
  /// This method returns:
  /// - `IMPOSSIBLE`
  ///   - if any of the date fields conflict, if a timestamp conflicts with any of the other fields,
  ///     or if the offset field is set but differs from the offset at that time in the `tz` time
  ///     zone.
  ...
  ///   the local time in the provided time zone is ambiguous (because it falls in a transition due
  ///   to for example DST) while there is no offset field or timestamp field set.
  /// - `OUT_OF_RANGE`
  ///   - if the value would be outside the range of a [`NaiveDateTime`] or [`FixedOffset`].
  ///   - if any of the fields of `Parsed` are set to a value beyond their acceptable range.
  ///   - if the date does not exist.
  pub fn to_datetime_with_timezone<Tz: TimeZone>(&self, tz: &Tz) -> ParseResult<DateTime<Tz>> {
    ...
    if let Some(timestamp) = self.timestamp {
      ...
      let dt = DateTime::from_timestamp(timestamp, nanosecond).ok_or(OUT_OF_RANGE)?.naive_utc();
...
fn resolve_week_date(year: i32, week: u32, weekday: Weekday, week_start_day: Weekday) -> ParseResult<NaiveDate> {
  if week > 53 {
    return Err(OUT_OF_RANGE);
  }
...
#[cfg(test)]
mod tests {
  use super::super::IMPOSSIBLE;
  use super::super::NOT_ENOUGH;
  ...
  fn test_parsed_to_naive_date() {
    ...
    assert_eq!(parse!(year_div_100: 19, year_mod_100: 83, month: 2, day: 29), Err(OUT_OF_RANGE));
    assert_eq!(parse!(year_div_100: 19, year_mod_100: 83, month: 13, day: 1), Err(OUT_OF_RANGE));
    assert_eq!(parse!(year_div_100: 19, year_mod_100: 83, month: 12, day: 31), ymd(1983, 12, 31));
    assert_eq!(parse!(year_div_100: 19, year_mod_100: 83, month: 12, day: 32), Err(OUT_OF_RANGE));
    assert_eq!(parse!(year_div_100: 19, year_mod_100: 83, month: 12, day: 0), Err(OUT_OF_RANGE));
    assert_eq!(parse!(year_div_100: 19, year_mod_100: 100, month: 1, day: 1), Err(OUT_OF_RANGE));
  ...
  fn test_parsed_to_naive_time() {
    ...
    assert_eq!(parse!(hour_div_12: 0, hour_mod_12: 1, minute: 23, second: 61), Err(OUT_OF_RANGE));
    assert_eq!(
  ...
  fn test_parsed_to_naive_datetime_with_offset() {
    ...
    let ymdhms = |y, m, d, h, n, s| Ok(NaiveDate::from_ymd_opt(y, m, d).unwrap().and_hms_opt(h, n, s).unwrap());
    ...
    // more timestamps
    let max_days_from_year_1970 = NaiveDate::MAX.signed_duration_since(NaiveDate::from_ymd_opt(1970, 1, 1).unwrap());
