...
//! `%f` counts the number of nanoseconds since the last whole second, while `%.f` is a fraction of
//! a second.<br>
...
impl<'a> StrftimeItems<'a> {
  ...
  ///   &[Item::Numeric(Numeric::Year, Pad::Zero), Item::Literal("-"), Item::Literal("%Q")];
  ...
  /// specifier and the [`StrftimeItems`] wasn't constructed with
  /// [`new_lenient`][Self::new_lenient].
  ///
  /// # Example
  ///
  /// ```
  ...
  fn parse_next_item(&mut self, mut remainder: &'a str) -> Option<(&'a str, Item<'a>)> {
    ...
    let (original, mut remainder) = match remainder.chars().next()? {
      ...
      c if c.is_whitespace() => {
        ...
        let nextspec = remainder.find(|c: char| !c.is_whitespace()).unwrap_or(remainder.len());
        assert!(nextspec > 0);
    ...
    let item = match spec {
      ...
      'v' => {
        queue![nums(Day), Literal("-"), fixed(Fixed::ShortMonthName), Literal("-"), num0(Year)]
      }
  ...
  #[cfg(feature = "unstable-locales")]
  fn switch_to_locale_str(&mut self, localized_fmt_str: impl Fn(Locale) -> &'static str, fallback: &'static [Item<'static>]) -> Item<'a> {
...
mod tests {
  ...
  use crate::format::fixed;
  use crate::format::internal_fixed;
  use crate::format::num;
  use crate::format::num0;
  use crate::format::nums;
