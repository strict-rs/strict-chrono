...
pub(super) fn nanosecond(s: &str) -> ParseResult<(&str, u32)> {
  ...
  const SCALE: [u32; 10] = [0, 100_000_000, 10_000_000, 1_000_000, 100_000, 10_000, 1_000, 100, 10, 1];
...
pub(super) fn nanosecond_fixed(s: &str, digits: usize) -> ParseResult<(&str, i64)> {
  ...
  // scale the number accordingly.
  static SCALE: [i64; 10] = [0, 100_000_000, 10_000_000, 1_000_000, 100_000, 10_000, 1_000, 100, 10, 1];
...
pub(super) fn short_or_long_month0(s: &str) -> ParseResult<(&str, u8)> {
  ...
  static LONG_MONTH_SUFFIXES: [&[u8]; 12] = [
    b"uary", b"ruary", b"ch", b"il", b"", b"e", b"y", b"ust", b"tember", b"ober", b"ember", b"ember",
  ];
...
pub(super) fn short_or_long_weekday(s: &str) -> ParseResult<(&str, Weekday)> {
  ...
  static LONG_WEEKDAY_SUFFIXES: [&[u8]; 7] = [b"day", b"sday", b"nesday", b"rsday", b"day", b"urday", b"day"];
...
pub(super) fn timezone_offset_2822(s: &str) -> ParseResult<(&str, i32)> {
  ...
  if upto > 0 {
    ...
    // named timezones.
    if name.eq_ignore_ascii_case(b"gmt") || name.eq_ignore_ascii_case(b"ut") || name.eq_ignore_ascii_case(b"z") {
...
mod tests {
  ...
  use super::short_or_long_month0;
  use super::short_or_long_weekday;
  use super::timezone_offset_2822;
  use crate::Weekday;
  use crate::format::INVALID;
  use crate::format::TOO_SHORT;
