...
use super::Numeric;
#[cfg(any(feature = "alloc", feature = "serde"))]
use super::OffsetFormat;
#[cfg(any(feature = "alloc", feature = "serde"))]
use super::OffsetPrecision;
#[cfg(any(feature = "alloc", feature = "serde"))]
use super::Pad;
#[cfg(feature = "alloc")]
use super::locales;
...
impl<'a, I: Iterator<Item = B> + Clone, B: Borrow<Item<'a>>> DelayedFormat<I> {
  ...
  #[must_use]
  pub fn new_with_locale(date: Option<NaiveDate>, time: Option<NaiveTime>, items: I, locale: Locale) -> DelayedFormat<I> {
    DelayedFormat {
      date,
      time,
      off: None,
  ...
  #[cfg(feature = "alloc")]
  fn format_numeric(&self, w: &mut (impl Write + ?Sized), spec: &Numeric, pad: Pad) -> fmt::Result {
    use self::Numeric::*;
    ...
    fn write_n(w: &mut (impl Write + ?Sized), n: usize, v: i64, pad: Pad, always_sign: bool) -> fmt::Result {
      if always_sign {
        match pad {
          Pad::None => write!(w, "{v:+}"),
    ...
    match (spec, self.date, self.time) {
      ...
      (IsoYearDiv100, Some(d), _) => write_two(w, d.iso_week().year().div_euclid(100) as u8, pad),
      (IsoYearMod100, Some(d), _) => write_two(w, d.iso_week().year().rem_euclid(100) as u8, pad),
      ...
      (Second, _, Some(t)) => write_two(w, (t.second() + t.nanosecond() / 1_000_000_000) as u8, pad),
      (Nanosecond, _, Some(t)) => write_n(w, 9, (t.nanosecond() % 1_000_000_000) as i64, pad, false),
      ...
      (Internal(_), ..) => Ok(()), // for future expansion
  ...
  fn format_fixed(&self, w: &mut (impl Write + ?Sized), spec: &Fixed) -> fmt::Result {
    ...
    match (spec, self.date, self.time, self.off.as_ref()) {
      (ShortMonthName, Some(d), ..) => w.write_str(short_months(self.locale)[d.month0() as usize]),
      (LongMonthName, Some(d), ..) => w.write_str(long_months(self.locale)[d.month0() as usize]),
      (ShortWeekdayName, Some(d), ..) => w.write_str(short_weekdays(self.locale)[d.weekday().num_days_from_sunday() as usize]),
      (LongWeekdayName, Some(d), ..) => w.write_str(long_weekdays(self.locale)[d.weekday().num_days_from_sunday() as usize]),
      ...
      (RFC3339, Some(d), Some(t), Some((_, off))) => write_rfc3339(w, crate::NaiveDateTime::new(d, t), *off, SecondsFormat::AutoSi, false),
      _ => Err(fmt::Error), // insufficient arguments for given format
    }
  }
}
...
impl OffsetFormat {
  ...
  fn format(&self, w: &mut (impl Write + ?Sized), off: FixedOffset) -> fmt::Result {
    ...
    let precision = match self.precision {
      ...
      OffsetPrecision::Seconds | OffsetPrecision::OptionalSeconds | OffsetPrecision::OptionalMinutesAndSeconds => {
        let minutes = off / 60;
        secs = (off % 60) as u8;
        mins = (minutes % 60) as u8;
        hours = (minutes / 60) as u8;
...
pub(crate) fn write_rfc2822(w: &mut (impl Write + ?Sized), dt: NaiveDateTime, off: FixedOffset) -> fmt::Result {
  let year = dt.year();
...
mod tests {
  ...
  fn test_date_format() {
    ...
    assert_eq!(NaiveDate::from_ymd_opt(12345, 1, 1).unwrap().format("%Y").to_string(), "+12345");
  ...
  fn test_offset_formatting() {
    fn check_all(precision: OffsetPrecision, expected: [[&str; 7]; 12]) {
      fn check(precision: OffsetPrecision, colons: Colons, padding: Pad, allow_zulu: bool, offsets: [FixedOffset; 7], expected: [&str; 7]) {
        let offset_format = OffsetFormat {
          precision,
          colons,
          allow_zulu,
    ...
    check_all(OffsetPrecision::Hours, [
    ...
    check_all(OffsetPrecision::Minutes, [
    ...
    check_all(OffsetPrecision::OptionalMinutes, [
    ...
    check_all(OffsetPrecision::OptionalSeconds, [
      ["+03:45", "-03:30", "+11:00", "-11:00:22", "+02:34:26", "-12:34:30", "+00:00"],
    ...
    check_all(OffsetPrecision::OptionalMinutesAndSeconds, [
  ...
  fn test_offset_minute_rounding_no_24h_rollover() {
    ...
    let dt = FixedOffset::east_opt(86_399)
      .unwrap()
